  ...
  char **argv)
{
  ...
  MPI_Gather(&host, hostlen, MPI_CHAR, (rchost[0]), hostlen, MPI_CHAR,
             MASTER, MPI_COMM_WORLD);
  MPI_Gather(&time, 1, mpi_timeval_t, rctime, 1, mpi_timeval_t, MASTER,
             MPI_COMM_WORLD);

  // Minimum finden
  MPI_Reduce(&(time.tv_usec), &min, 1, MPI_LONG, MPI_MIN, MASTER,
             MPI_COMM_WORLD);
