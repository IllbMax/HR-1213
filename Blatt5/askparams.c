...
AskParams (struct options* options, int argc, char** argv)
{
	...
	if (argc < 2)
	{
		...
		while (ret != 1 || (options->method != METH_GAUSS_SEIDEL && options->method != METH_JACOBI));
		...
		while (ret != 1 || options->interlines < 0 || options->interlines > 1000);
		...
		while (ret != 1 || (options->inf_func != FUNC_F0 && options->inf_func != FUNC_FPISIN));
		...
		while (ret != 1 || (options->termination != TERM_PREC && options->termination != TERM_ITER));
		...
		if (options->termination == TERM_PREC)
		{
			...
			while (ret != 1 || options->term_precision < 1e-20 || options->term_precision > 1e-4);
		...
		else if (options->termination == TERM_ITER)
		{
			...
			while (ret != 1 || options->term_iteration < 1 || options->term_iteration > MAX_ITERATION);
